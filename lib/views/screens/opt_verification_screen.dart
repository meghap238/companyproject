...
class _OptVerificationScreenState extends State<OptVerificationScreen> {
  ...
  String? otp;
  String? dvcId;
  int? mobileNumber;  // Changed from strigNumer to mobileNumber for clarity
  Timer? timer;
  ...
  Future<void> getMobileNum() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      mobileNumber = prefs.getInt('mobile') ?? 0; // Safely get the mobile number or set it to 0
      dvcId = prefs.getString('deviceId');
    });
                      ...
                      children: [
                            ...
                            Flexible(
                              child: FittedBox(
                                ...
                                child: Text(
                                  ...
                                  style: AppStyle.blackClrtxtSyle(fontWeight: FontWeight.w700),
                                ),
                              ),
                            ),
                          ],
                        ...
                        Padding(
                          ...
                          child: Text(
                            '$errorMsg',
                            style: AppStyle.blackClrtxtSyle(
                                color: Colors.red, fontSize: 10.sp, fontWeight: FontWeight.w600),
                            textAlign: TextAlign.center,
                          ),
                        ),
                        SizedBox(height: 20.sp),
  ...
  Widget _buildOtpField(TextEditingController controller, FocusNode currentNode, FocusNode? nextNode) {
    return Flexible(
      child: TextFormField(
        controller: controller,
...
if(widget.isForgot){
FPasswordVerifyOtpModel? otprsp = await ApiService().FVerifyforgot(otp: int.parse(otp), deviceId: dvcId!);
if (otprsp?.status == 200) {
  ...
  VerifiedOtpModel? otpResponse = await ApiService().verifiedOtpApi(
    otp: int.parse(otp),
    deviceId: dvcId!,
  );
